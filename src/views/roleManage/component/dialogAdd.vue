<template>
	<div>
		<div>
			<span>角色名称:</span>
			<el-input v-model.trim="queryParams.name" clearable class="input-search" size="small"
				placeholder="商户ID/商户名称"></el-input>
		</div>
		<div class="mt10">
			<span>角色类型:</span>
			<el-select v-model="queryParams.type" placeholder="请选择" size="small">
				<el-option v-for="item in this.$constants.roleType" :key="item.value" :label="item.name" :value="item.value">
				</el-option>
			</el-select>
		</div>
		<div class="mt20">
			<h2>权限清单：</h2>
			<div>
				<el-tree :data="permissionList" ref="roleTree" show-checkbox node-key="value" :default-expand-all="true"
					:default-checked-keys="queryParams.permissoinItemIds" :props="defaultProps">
				</el-tree>
			</div>
